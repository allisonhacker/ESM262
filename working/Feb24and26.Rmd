...
possible_cars2 <- data.frame(name = c("A","B","C"),mass=c(31000,45000,38000), area = c(25,30,22))

possible_cars2$power = autopower(V=speed_base2, A = possible_cars2$area, m=possible_cars2$mass)
```
...
possible_cars2$prob = c(0.4, 0.4, 0.2)
...
possible_cars2$row = seq(from=1, to=nrow(possible_cars2))
whichcar = base::sample(possible_cars2$row, size=nsample, prob=possible_cars2$prob, replace=TRUE)

results2$mass <- possible_cars2$mass[whichcar]
...
rownames(npvs)=damages
 ...
 ggplot(npv_total, aes(dis,t, fill=dis))+geom_col() + labs(x="Discount Rate", y="Total ($)")
...
# will run forever if the condition is not met. Add a condition to keep the loop in check
...
while (pollutant_level < 30 && yr < 1000)   {
  # increase pollutant
pollutant_level = pollutant_level + 0.0001* pollutant_level 
# keep track of time
yr = yr + 1
}
...
sprintf("There are %5.4f things", num)
```
...
source("../../ESM262/R/compute_simpson_index.R")
...
# works but need to index within cost to get a single value
...
sage = read.table("../../ESM262/data/sagedata.txt", header=T)
names(sage)
...
# repeat with a list
source("../../ESM262/R/computediversity.R")
...
# test function on flowers data
flowers = read.table("../../ESM262/data/flowers.txt")
computediversity(flowers$V1)
head(flowers)
```
...
source("../../ESM262/R/compute_season_meanflow.R")
